...
.hero {{
  ...
  background:
    linear-gradient(90deg, rgba(3,22,35,.94) 0%, rgba(3,22,35,.77) 42%, rgba(3,22,35,.25) 73%, rgba(3,22,35,.08) 100%),
    linear-gradient(0deg, rgba(3,22,35,.78) 0%, rgba(3,22,35,0) 48%),
    url('assets/healthcare-fleet-hero.png') center 48%/cover no-repeat;
}}
.hero::before {{
  content: "";
  position: absolute;
...
.hero::after {{
  ...
  z-index: -1;
  background:
    radial-gradient(circle at 76% 38%, rgba(56,198,255,.19), transparent 27%),
...
.hero-shell {{
  ...
  margin: 0 auto;
  display: grid;
  grid-template-columns: minmax(0, 1.22fr) minmax(320px, .62fr);
  gap: clamp(24px, 5vw, 80px);
  align-items: end;
  padding-bottom: clamp(34px, 7vh, 82px);
}}
.hero-inner {{ max-width: 900px; }}
.brand-lockup {{ display: flex; align-items: center; gap: 18px; flex-wrap: wrap; margin-bottom: 26px; }}
.hero-logo {{ width: min(300px, 58vw); height: auto; filter: drop-shadow(0 12px 30px rgba(0,0,0,.42)); }}
.hero-kicker {{
  display: inline-flex; align-items: center; gap: 9px; margin: 0 0 14px; padding: 7px 11px;
  border: 1px solid rgba(255,255,255,.26); border-radius: 999px; background: rgba(3,22,35,.32);
  backdrop-filter: blur(12px); font-size: 12px; font-weight: 900; letter-spacing: .16em; text-transform: uppercase;
}}
.hero-kicker::before {{ content: ""; width: 8px; height: 8px; border-radius: 50%; background: var(--cyan); box-shadow: 0 0 18px var(--cyan); }}
.hero h1 {{
  font-size: clamp(48px, 7vw, 96px); line-height: .93; margin: 0 0 22px; letter-spacing: -.045em;
  max-width: 980px; text-shadow: 0 14px 38px rgba(0,0,0,.38);
}}
.hero p {{
  font-size: clamp(18px, 2vw, 23px); max-width: 830px; line-height: 1.48; margin: 0 0 30px;
  color: rgba(255,255,255,.92); text-shadow: 0 3px 20px rgba(0,0,0,.38);
}}
...
.hero-card-brand {{
  ...
  padding-bottom: 15px; margin-bottom: 15px; border-bottom: 1px solid rgba(255,255,255,.14);
}}
.hero-card-brand img {{ width: 118px; height: auto; }}
.hero-live {{ display: inline-flex; align-items: center; gap: 7px; font-size: 11px; font-weight: 900; letter-spacing: .12em; text-transform: uppercase; color: rgba(255,255,255,.8); }}
.hero-live i {{ width: 8px; height: 8px; border-radius: 50%; background: var(--green); box-shadow: 0 0 16px var(--green); }}
.hero-stats {{ position: relative; display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 10px; }}
.hero-stat {{ min-height: 104px; padding: 14px; border-radius: 12px; background: rgba(255,255,255,.075); border: 1px solid rgba(255,255,255,.11); }}
.hero-stat span {{ display: block; color: rgba(255,255,255,.64); font-size: 10px; font-weight: 900; letter-spacing: .1em; text-transform: uppercase; }}
.hero-stat strong {{ display: block; margin-top: 7px; font-size: 29px; line-height: 1; }}
.hero-stat small {{ display: block; margin-top: 7px; color: rgba(255,255,255,.68); line-height: 1.2; }}
.hero-footnote {{ position: relative; display: flex; align-items: center; gap: 10px; margin-top: 14px; color: rgba(255,255,255,.7); font-size: 11px; }}
.hero-footnote img {{ width: 62px; height: auto; opacity: .9; }}
...
@media (max-width: 1050px) {{ .hero-shell {{ grid-template-columns: 1fr; }} .hero-command-card {{ max-width: 680px; }} .metrics {{ grid-template-columns: repeat(2, 1fr); }} .layout, .charts, .ops-grid, .command-strip, .action-grid, .risk-explainer, .route-explainer {{ grid-template-columns: 1fr; }} #map {{ height: 520px; }} }}
...
<section class="hero">
  <div class="hero-shell">
    <div class="hero-inner">
      ...
      <div class="hero-kicker">RPS Medical Fleet Operations</div>
      <h1>Fleet Command</h1>
      <p>Supervisión integral de vehículos, rutas, paradas y eventos de velocidad para operaciones de salud y entregas críticas en Puerto Rico.</p>
    ...
    <aside class="hero-command-card" aria-label="Resumen operacional RPS">
      <div class="hero-card-brand">
        <img src="assets/rps-logo-white.png" alt="RPS Medical">
        <span class="hero-live"><i></i> Sistema activo</span>
      </div>
      <div class="hero-stats">
        <div class="hero-stat"><span>Vehículos</span><strong id="heroVehicles">—</strong><small>unidades monitoreadas</small></div>
        <div class="hero-stat"><span>Rutas</span><strong id="heroRoutes">—</strong><small>viajes analizados</small></div>
        <div class="hero-stat"><span>Excesos</span><strong id="heroSpeeding">—</strong><small>eventos detectados</small></div>
        <div class="hero-stat"><span>Actualización</span><strong>5 min</strong><small>ciclo de publicación</small></div>
      </div>
      <div class="hero-footnote"><img src="assets/rps-logo-white.png" alt=""><span>Operational intelligence for critical healthcare delivery.</span></div>
    </aside>
  </div>
</section>
<nav>
...
function renderHeroSnapshot(r) {{
  document.getElementById('heroVehicles').textContent = r.vehicles.length;
  document.getElementById('heroRoutes').textContent = r.trips.length;
  document.getElementById('heroSpeeding').textContent = r.speeding.length;
}}
function renderHeroSnapshot(r) {{
  document.getElementById('heroVehicles').textContent = r.vehicles.length;
  document.getElementById('heroRoutes').textContent = r.trips.length;
...
function render() {{ const r = rows(); renderHeroSnapshot(r); renderLastUpdated(); renderMetrics(r); renderRiskExplainer(r); renderRouteExplainer(r); renderCommandStrip(r); renderActionPlan(r); renderMap(r); renderCards(r); renderCharts(r); renderRecs(); renderOps(r); renderTable(tableName, r); }}
